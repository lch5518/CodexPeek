...
- [ ] Hover the taskbar and floating widgets in light and dark mode; verify the 400-DIP popup
      stays inside the active monitor and groups each window's percentage, bar, and reset time,
      followed by pace, forecast, and the daily token chart. Verify that widths below 320 DIP
      stack each row's label and value, stale values show status and last success, and the popup
...
- [ ] 프로필 관리/추가 창이 Windows 10/11 밝은·어두운 모드에서 네이티브 팔레트를 공유한다.
      프로필 이름, 파란 선택 면·선, 얇은 행 구분선, 파란 주 버튼을 확인한다.
