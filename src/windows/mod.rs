...
/// 프로필 관리 화면에 전달할 사용량과 메모리 전용 로그인 이메일입니다.
///
/// `account_email`은 관리자 화면에서만 표시하고 파일·로그에 기록하지 않습니다. 내부 경로와
/// 인증 토큰은 포함하지 않으며 `id`는 UI 동작을 런타임 프로필에 연결하는 데만 사용합니다.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct UsageProfileView {
    /// 마지막 유효한 조회에서 확인한 이메일입니다. 로그인 필요·오래된 조회 상태에는 없습니다.
