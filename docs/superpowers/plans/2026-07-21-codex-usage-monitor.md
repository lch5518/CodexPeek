...
**Files:** create `src/main.rs`, `src/app.rs`, `src/windows/mod.rs`, `src/windows/native.rs`, `src/windows/tray.rs`, `src/windows/widget.rs`, `src/windows/taskbar.rs`, and `src/windows/autostart.rs`; update `src/lib.rs`, `Cargo.toml`, and Codex diagnostics exposure only where required. Keep raw Win32 unsafe code inside `src/windows` where practical.

Build a Windows-subsystem binary with Per-Monitor DPI Awareness V2, a named mutex single-instance guard, a hidden message window, and a UI-thread Win32 message loop. `--startup` respects `startup_view` and `widget_visible`; normal startup uses the saved mode. `--diagnose` attaches/writes to the parent console (and logs safely), performs bounded CLI/app-server/login/response-format, settings, proxy-presence, and taskbar-discovery checks, then exits without starting the tray loop. It may report the auth path and existence only and must never read auth contents or print environment proxy values.

Implement a tray owner that adds/removes `NOTIFYICONDATAW`, uses a dynamically drawn original meter/badge icon based on the higher available usage percentage, restores itself after the registered `TaskbarCreated` message, and opens a checked context menu. Menu actions must cover refresh, taskbar/floating display mode, 1/5/10/15/30-minute interval, start with Windows, startup view, forced authentication refresh, automatic auth refresh, always-on-top, Korean/English/automatic language, reset position, diagnostics, update check, widget show/hide, and exit. Menu IDs map through a pure typed `UiAction` function tested independently.

The floating widget is a borderless 380x112 logical-pixel tool window. It renders both usage windows, status/last-success text, percentage text, progress bars, and stable/normal/caution/danger/limited shapes using Segoe UI and GDI. It supports drag-to-move, persists logical coordinates and monitor device, clamps an unavailable/off-screen saved position into the nearest monitor work area, honors always-on-top, and recomputes layout on `WM_DPICHANGED`. Pure layout tests cover 100/125/150/200 percent DPI and progress/status bounds.

The taskbar widget is a 380x48 logical-pixel child rendering the two compact usage rows. Enumerate `Shell_TrayWnd` and `Shell_SecondaryTrayWnd`, choose the saved monitor when possible, reject vertical taskbars, find the notification area for collision-free placement, call `SetParent`, and verify `GetParent`. Monitor the parent/notification geometry on a timer and handle the registered `TaskbarCreated` message; an invalid parent, Explorer restart, vertical orientation, or attach/placement failure must detach safely and show the floating widget instead. `SetWindowLongPtrW` style transitions and all Win32 handles/icons/fonts must have explicit ownership and cleanup. Pure taskbar-geometry tests cover primary/secondary horizontal placement, offsets, vertical rejection, and insufficient space.

Implement HKCU autostart under `Software\\Microsoft\\Windows\\CurrentVersion\\Run`, value `CodexUsageMonitor`. The stored command is exactly a safely quoted current executable path plus `--startup`; reject paths containing a quote, write `REG_SZ`, read it back for exact verification, and remove it on disable. Put command construction and a `RegistryBackend` abstraction under tests; production uses Win32 registry APIs and never requests elevation.

Wire `PollingService`, `SettingsStore`, `DiagnosticLogger`, and `UpdateChecker` only in `app.rs`; windows rendering modules consume immutable view models/snapshots and emit typed actions. Update checks run off the UI thread and open only the checker-validated GitHub tag page via `ShellExecuteW`; no asset is downloaded. A one-second timer invalidates the widget, updates the tray badge/status, recomputes stale text, and performs taskbar recovery without starting overlapping RPC calls.

Run focused RED/GREEN tests, `cargo test --all-targets`, `cargo fmt --check`, `cargo clippy --all-targets --all-features -- -D warnings`, `cargo build`, and smoke `--diagnose` before committing `feat: add native windows application`.
