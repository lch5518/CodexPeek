...
시스템 프로필을 포함해 전체 8개까지 사용할 수 있습니다. 트레이의 **사용량 프로필** 하위 메뉴에서는
프로필을 선택하거나 **사용량 프로필 관리**만 열 수 있으며, 추가 명령은 없습니다. 추가와 선택 방법은
다음과 같습니다.

1. 트레이 메뉴에서 **사용량 프로필 → 사용량 프로필 관리**를 열고, 관리자 목록 아래의 `+`를
   선택합니다. 새 프로필은 이 위치에서만 추가합니다.
...
관리자 창은 아래쪽에 닫기 또는 추가 단추가 없으며 `X` 또는 Esc로 닫습니다. 시스템 프로필 이름은
바꿀 수 있고 사용자 지정 이름은 CodexPeek 표시만 바꾸며 계정 식별자가 아닙니다. 기본 계정 표시는
관리자에서만 보입니다. 로그아웃과 다시 로그인은 관리 프로필에만 적용됩니다. 삭제할
때는 로컬 프로필 데이터와 그 안에 별도로 저장된 CLI 인증 정보를 복구할 수 없다는 확인 안내를
읽으세요. 삭제에 실패하면 프로필과 원래 디렉터리를 보존해 다시 시도할 수 있으며, 완료된 삭제의
내부 정리 항목은 다음 시작 시 안전하게 복구·정리됩니다. 기본 Codex 계정 프로필은 로그아웃하거나
삭제할 수 없습니다.
...
설정을 적용합니다. Windows 사용자·시스템 환경, 기본 Codex 홈, 터미널 및 IDE 설정과 Codex CLI
로그인 상태는 수정하지 않습니다. `--diagnose`와 진단 로그는 프로필별 이름·경로·계정 정보 대신 설정 및 결과 개수만
집계합니다.
...
실행 중에는 24시간마다 재확인하며, 확인 실패 시 15분 뒤 재시도합니다. 위젯을 숨겨도 확인은 계속됩니다.
0.4.7에서는 시작 시에만 자동 확인하므로, 계속 켜둔 앱은 트레이의 **업데이트 확인**을 사용하거나
앱을 종료 후 다시 실행해야 새 릴리스를 확인합니다.
...
helper는 새 앱이 트레이와 초기 창을 만든 것을 확인한 뒤에만 이전 EXE 백업을 삭제합니다. 새 앱이
초기화 중 종료되거나 제한 시간 안에 준비되지 않으면 해당 프로세스의 종료를 확인한 뒤 이전 EXE를
복원합니다. 종료를 확인할 수 없으면 백업을 보존하고 rollback이나 두 번째 실행을 하지 않습니다.
Windows 자동 시작으로 실행한 경우 `--startup` 모드도 그대로 전달합니다. Installer 버전을 인앱 업데이트하면 Windows 앱
목록의 표시 버전은 다음 Setup 설치 전까지 기존 Installer 버전으로 남을 수 있습니다.
