...
- 위젯에 마우스를 올리면 최대 14일의 일별 토큰 사용량을 막대로 표시합니다.
  날짜는 해당 막대에 맞춰 배치하며, 0 토큰인 날에는 기준선만 표시합니다.
- Codex 기본·보조 사용량 기간과 초기화 시각을 표시합니다.
- 400 DIP 호버 패널에서 각 사용량 창의 비율·막대·초기화 시각을 함께 표시하고,
  소비 속도·예측·일별 토큰 그래프를 이어서 보여 줍니다. 우클릭 메뉴와 프로필 관리는
  시스템 글꼴과 파란 선택·작업 표시를 사용하며 사용량 상태색은 별도로 유지합니다.
...
프로필 표시명은 사용자가 직접 지정합니다. 프로필 관리 화면은 이름 아래에 `account/read`에서
확인한 로그인 이메일을 표시합니다. 이메일은 메모리에만 보관하며 설정·사용량 기록·로그에 저장하지
않습니다. 정보가 없거나 오래되거나 조회에 실패하면 `—`를 표시하고 로그인·로그아웃 시 이전
이메일을 지웁니다. 로그인할 때 브라우저에서 사용할 ChatGPT 계정을 직접 확인하세요. 프로필
...
우클릭 메뉴 최상단에서 `CodexPeek v0.4.8`처럼 현재 실행 중인 앱 버전을 확인할 수 있습니다.

수동 실행과 Windows 자동 시작 모두 앱 화면이 먼저 열린 뒤 최신 GitHub Release를 확인합니다.
실행 중에도 24시간마다 확인하며, 확인 실패 시 15분 뒤 재시도합니다. 위젯이 숨겨져 있어도 동작합니다.
...
토큰, 계정 ID, 인증 파일 내용, 프록시 값은 보관하거나 로그에 기록하지 않습니다. 로그인 이메일은 프로필 관리 화면용으로 메모리에만 보관하고 Debug 출력·파일·로그에는 남기지 않습니다.
